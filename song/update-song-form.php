<?php
    ...
    $query = "SELECT song_id, pub_song_id, album_id, song_name, song_rating, song_role, song_high_path,lyric,publisher_id FROM songs WHERE song_id = " .$song_id;
...
<form id="update-song-form" method="POST" enctype="multipart/form-data" action="#">
            ...
            <label for='publishername'>Publisher Name</label>
                <?php
                    echo '<select id="publisher" class="form-control" name="publisher">';
                       $query = "SELECT pub_id,pub_name FROM publisher";
                       $stmt = $db_con->prepare($query);
                       $stmt->execute();
                       $num = $stmt->rowCount();
                    if ($num > 0) {
                        while ($pub_row = $stmt->fetch(PDO::FETCH_ASSOC)){
                            ?>

                            <option value="<?php echo $pub_row['pub_id']?>" <?php echo ($pub_row['pub_id'] == $pub_id) ? 'selected' : ''; ?> ><?php echo $pub_row['pub_name'];?>
                            </option>
        ...
        <input type="hidden" id="pub_song_id" name="pub_song_id" value="<?php echo $pub_song_id ?>"/>
</form>
